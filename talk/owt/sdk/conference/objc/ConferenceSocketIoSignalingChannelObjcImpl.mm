...
void ConferenceSocketSignalingChannel::Connect(
  ...
  NSString *host_ns_str = [NSString stringWithCString:scheme.append(host).c_str() encoding:[NSString defaultCStringEncoding]];
  NSDictionary *config = @{kReconnectAttemptsKeyName: [NSNumber numberWithInt:kReconnectionAttempts],
                           kReconnectWaitKeyName: [NSNumber numberWithInt:kReconnectionDelay],
                           kReconnectWaitMaxKeyName: [NSNumber numberWithInt:kReconnectionDelayMax];
  [signaling_channel_ setup:config];

  std::weak_ptr<ConferenceSocketSignalingChannel> weak_this =
      shared_from_this();

  [signaling_channel_ setSocketConnectClosedBlock:^{
  ...
  [signaling_channel_ setSocketConnectFailedBlock:^(NSError * _Nonnull error) {
  ...
  [signaling_channel_ setSocketReconnectBlock:^{
  ...
  [_signalingChannel setSocketReconnectingBlock:^{
  ...
  [signaling_channel_ setSocketConnectSuccessBlock:^{
    ...
    if (!is_reconnection_) {
      ...
      NSString* reconnectionTicket = [NSString stringWithCString:reconnection_ticket_.c_str() encoding:[NSString defaultCStringEncoding]];
      [_signalingChannel emit:kEventNameRelogin message:@[reconnectionTicket] callback:^(NSArray * _Nonnull data) {
        auto that = weak_this.lock();
        if (!that) {
          return;
        }
